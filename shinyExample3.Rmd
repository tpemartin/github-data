...
library(flexdashboard);library(dplyr); library(stringr); library(lubridate); library(purrr); library(ggplot2); library(plotly); library(readr);
```
...
```{r quote1}
quote({
  .x<- input$student_id  # readline(prompt="學號")
  studentData %>%
...
```{r}
renderPlotly(exprObj2,quoted=TRUE)
```
